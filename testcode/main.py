...
import argparse
import os
import coin_recognition_SIFT as model

def predict_input_fn(histograms):
    return tf.data.Dataset.from_tensor_slices( (histograms, [0 for _ in range(histograms.shape[0])] ))
...
def predict_whole_images():
    ...
    print ("Going to load bagOfWords.")
    bagOfWords.load_model(os.path.join(model._MODEL_DIR, "SIFT_bagofwords.pkl"))
    print ("Load bagOfWords Done.")

    files = [
        '/Users/maxtang/Projects/study/vip/image/multi/20180416_225348.jpg'
    ]
    # files = ['/Users/maxtang/Downloads/22042018/20180422_214123.jpg']
    ...
    files = [os.path.join('multi', f) for f in os.listdir('multi') if os.path.isfile(os.path.join('multi', f)) and (f.endswith('.jpg') or f.endswith(".jpeg"))]
    ...
    for f in files:
        ...
        for i in circles[0,:]:
            ...
            cv2.imwrite("tmp/{}.png".format(idx), coin_img)

            histogram = bagOfWords.construct_feature_map([coin_img])
...
def predict_good_quality_coins():
    ...
    for f in files:
        ...
        histogram = bagOfWords.construct_feature_map([img])
        predictions = list(classifier.predict(input_fn=lambda:predict_input_fn(histograms=histogram)))
